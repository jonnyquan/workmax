...
### 0.5.24 外链修复的验证，以及同源形态的第三个后果（2026-08-08，rev.19）

§0.5.23 的外链修复是**靠推理写的，没有实测**。补验的过程本身有产出。

**第一次尝试：从宿主驱动页面。** Wails 有 `ExecJS`，可以像用户点鼠标一样触发点击，而不必往出货页面注入探针。结果 `--verify-app` 报"外链未到达 Go"——**但那是 harness 误判**。加一个标记请求区分之后真相是：**`ExecJS` 根本没执行**。

源码里写得很清楚：`ExecJS` 在 `runtimeLoaded` 为假时**排队而不执行**，而 runtime 的就绪信号要经它自己的 `/wails/runtime` 通道——正是 §0.5.11 实测在 loopback origin 上不通的那个。试过把 `/wails/*` 从 UI origin 代理回 Wails 的 asset handler（§0.5.11 列的方案 (a)），仍未唤醒 runtime。代理代码已撤除：**没兑现的复杂度比一个记录在案的缺口更糟**。

**结论——同源形态的第三个后果**：不只是绑定不可用（§0.5.11），**宿主完全无法驱动页面**。`ExecJS`、绑定、事件回传都走同一条已断的通道。这不影响用户功能（renderer 通过同源 HTTP 拿到它需要的一切），但意味着**任何"从 Go 驱动 UI"的设计都行不通**，将来要做托盘菜单触发 UI 动作之类的功能时会撞上。

**验证换了地方，而且换对了**：外链拦截的测试放进行为套件——它本来就在 VM 里跑页面 JS，能直接派发点击，**而且能进 CI**（真 webview 不能）。断言三条：捕获阶段注册（否则 `stopPropagation` 能绕过）、外部链接被 `preventDefault` 且 URL 交给 Go、同源与片段链接不受影响。三条都做了负向验证。

> 这一段的教训：**"没法测"往往是"测的位置不对"**。往宿主要能力失败之后，正确的反应不是接受未验证，而是找到那个能跑的层。

### 0.5.25 补齐两份操作文档（2026-08-08，rev.20）

**安全对等清单**（`design/wails-security-parity-2026-08.md`）——§14/§16 三处把它列为 cutover 门槛，而它从未存在。现已写成，规则是**每条只写实际验证过的**：文件位置 + 自动化断言，结论限"保持 / 加强 / 降级"三种。**"设计如此但没验证"不算结论**——那种行在这份清单里不存在，因为 rev.18 的外链缺口正是这么产生的（文档四处声称有，代码里没有）。

清单如实记了三处降级或未完成：导航收束（mac 无阻止式钩子，顶层导航无法阻止且回退未实现）、D3 与原设计的偏差、签名公证未实跑。另附评审结论栏，和三个建议追问的问题——第一个就是"**还有哪种触发导航的形态没被枚举**"，因为那正是外链那次漏掉的。

**`SMOKE_CHECKLIST.md`**——50 处引用已删除的东西，其中 §3/§4 把 `smoke-packaged-app.sh` 当作公开发布门槛，而**那个脚本已随 Electron 删除**：一个无法执行的门槛。已按现有工具重新组合出可跑的打包门槛（`build-mac.sh` → 自动过 inspector → 对**打包产物**跑 `--verify-app`/`--verify-shim`/`--kill-check` → `--serve-only` + `smoke-local.sh`），清单里每条命令都实跑验证过。

它唯一无法覆盖的是"已认证缓存 + 云端不可达"的离线场景（需要真实云会话），**如实标为无 helper 的手工门槛**，而不是悄悄从清单里去掉。
...
- **安全对等清单**：✅ **已写**，见 `ProjectDocs/design/wails-security-parity-2026-08.md`。逐条对照附录 C，每条只写实际验证过的（文件位置 + 自动化断言），结论限"保持/加强/降级"三种——**"设计如此但没验证"不算一种结论，那样的行不存在**。含 D4.1 逐字写入、导航收束的降级与残余、D3 与原设计的偏差、以及签名公证未实跑这一未完成项。附评审结论栏与三个建议追问的问题。
...
- 切换门槛（首版）：契约对等 ✓、性能基准落数字 ✓、安全对等清单**已写待评审**（`design/wails-security-parity-2026-08.md`）、至少 1 个 **arm64** EA 构建签名公证 ⏸（需证书，未实跑）。**RAG 与 x64 不是切换门槛**——它们在 Electron 通道上今天也不具备（RAG）或不受影响（x64 需在 M+1 补齐后才能对 Intel 用户 cutover）。
...
| **rev.20** | **2026-08-08** | **补齐两份被引用却不存在的操作文档（§0.5.25）。** 新增 `design/wails-security-parity-2026-08.md`——§14/§16 三处列为 cutover 门槛的安全对等清单，逐条对照附录 C，只写验证过的，结论限保持/加强/降级三种，如实记录三处降级或未完成。重写 `SMOKE_CHECKLIST.md`：50 处失效引用，其中公开发布门槛依赖的 `smoke-packaged-app.sh` 已随 Electron 删除；改用现有工具重组可跑门槛并实跑验证，无法覆盖的离线认证场景如实标为手工 |
| rev.19 | 2026-08-08 | **验证 rev.18 的外链修复（§0.5.24）。** 发现同源形态的第三个后果：**宿主无法驱动页面**——`ExecJS` 排队等 runtime 就绪，而 runtime 在 loopback origin 上永不就绪（与绑定同源），代理 `/wails/*` 也未能唤醒；试探代码已撤除。外链拦截测试改放行为套件（能进 CI），三条断言均经负向验证 |
